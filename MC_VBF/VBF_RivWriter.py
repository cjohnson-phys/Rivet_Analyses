...
f = open('/data/atlas/atlasdata/henderson/custom_rivet/MC_VBF.cc', 'w')
...
namespace Rivet {
  ...
  /// Default constructor
    ...
    void init() {
      ...
      FinalState fs;
      WFinder wfinder(fs, -2.5, 2.5, 25.0*TeV, """
f.write(section)
...
section = """60.0*TeV, 100.0*TeV, 25.0*TeV, 0.2);
  addProjection(wfinder, "WFinder");

  ZFinder zfinder(fs, -2.5, 2.5, 25.0*TeV, """
...
section = """65.0*TeV, 115.0*TeV, 0.2, true, true);
  ...
  void analyze(const Event& event) {
   ...
   if ( zfinder.bosons().size() == 1 && wfinder.bosons().size() == 0 ) {
      isZ = true;
      all_jets      = applyProjection<FastJets>(event, "Jets_z").jetsByPt(30.0*TeV);
      ...
      isZ = false;
      all_jets = applyProjection<FastJets>(event, "Jets_w").jetsByPt(30.0*TeV);
...
if options.dijetMassCut:
    section="""
    if ( sqrt(fabs(dijet.mass2()))/TeV < """+str(options.dijetMassCut)+""" ) vetoEvent;
    """
    f.write(section)
    pass
...
if options.cuts:
    ...
    if ( jet1.pT()/TeV < 80 )                                                 vetoEvent;
    if ( jet2.pT()/TeV < 60 )                                                 vetoEvent;
    if ( lepton.pT()/TeV < 25  )                                              vetoEvent;
    // TRANSVERSE MASS CUT!!
    if ( sqrt(fabs(dijet.mass2()))/TeV < 500 )                                vetoEvent;
    """
...
section="""
	...
	_h_PID->fill(abs(gp->pdg_id()), weight);
      ...
      _h_jet1_pT->fill(  jet1.pT()/TeV, weight);
      _h_jet2_phi->fill( acos(cos(jet2.phi())), weight);
      _h_jet2_eta->fill( jet2.eta(), weight);
      _h_jet2_pT->fill(  jet2.pT() /TeV, weight);
 
      if ( my_jets.size() > 2 ){
	_h_jet3_phi->fill( acos(cos(jet3.phi())), weight);
	_h_jet3_eta->fill( jet3.eta(), weight);
	_h_jet3_pT->fill(  jet3.pT() /TeV, weight);
        _h_Zeppenfeld->fill( (jet3.eta() - ( jet1.eta() + jet2.eta() ) / 2.0)   , weight);
      }

      _h_mjj->fill( sqrt(fabs(dijet.mass2())) /TeV, weight);
      _h_ALTmjj->fill( sqrt(fabs(dijet.mass2())) /TeV, weight);
      _h_zoomed_mjj->fill( sqrt(fabs(dijet.mass2())) /TeV, weight);
      _h_RATIOmjj->fill( sqrt(fabs(dijet.mass2())) /TeV, weight);
      _h_scalarSumPt->fill( ( jet1.pT() + jet2.pT() ) /TeV, weight);
      _h_vectorSumPt->fill( dijet.pT() /TeV, weight);
      _h_etaProduct->fill( (jet1.eta() * jet2.eta()), weight);
      _h_dijetPt->fill( dijet.pT() /TeV, weight);
      _h_RATIOdijetPt->fill( dijet.pT() /TeV, weight);
      _h_ALTdijet_pT->fill( dijet.pT() /TeV, weight);
      _h_gapref->fill( ((jet1.pT() + jet2.pT())/2)  /TeV, weight);
      ...
      double mT_dilep = sqrt( boson.Et2() - boson.pT2() ) /TeV;	
	...
	_h_njet_inclusive->fill(n_jet, weight);
      ...
      _h_BDPtSum->fill( (boson.px() + dijet.px()) /TeV + (boson.py() + dijet.py()) /TeV, weight);
      _h_MomFac->fill( MomX * MomY, weight);
      _h_DPtInW->fill(boson.pT()/TeV + dijet.pT()/TeV *cos( boson.phi() - dijet.phi() ), weight);
      if( sqrt(dijet.mass2()) /TeV > 350){
        _h_BDPtSum350->fill( (boson.px() + dijet.px())/TeV + (boson.py() + dijet.py())/TeV, weight);
        _h_MomFac350->fill( MomX * MomY, weight);
        _h_DPtInW350->fill(boson.pT()/TeV + dijet.pT()/TeV *cos( boson.phi() - dijet.phi() ), weight);
        if ( my_jets.size() == 2) _h_BDPtSum2Jets->fill( (boson.px()/TeV + dijet.px()/TeV) + (boson.py()/TeV + dijet.py()/TeV),  weight);  
        if( sqrt(dijet.mass2()) /TeV > 1000){
          _h_BDPtSum1000->fill( (boson.px()/TeV + dijet.px()/TeV) + (boson.py()/TeV + dijet.py()/TeV), weight);
          _h_MomFac1000->fill( MomX * MomY, weight);
          _h_DPtInW1000->fill(boson.pT()/TeV + dijet.pT()/TeV*cos( boson.phi() - dijet.phi() ) , weight);
        }      
      }
      ...
      if ( sqrt(fabs(dijet.mass2())) /TeV > 500 && my_jets.at(0).momentum().pT() /TeV > 80 && my_jets.at(1).momentum().pT() /TeV > 60 ){
      ...
      _h_lepton_pT->fill(lepton.pT()/TeV, weight);
      
      if ( isZ ) {
      _h_neutrino_phi->fill( acos(cos(second_lepton.phi())), weight);
      _h_neutrino_eta->fill(second_lepton.eta(), weight);
      _h_neutrino_pT->fill(second_lepton.pT() /TeV, weight);
      }
      else {
        _h_neutrino_phi->fill( acos(cos(neutrino.phi())), weight); 
        _h_neutrino_eta->fill(neutrino.eta(), weight);
        _h_neutrino_pT->fill(neutrino.pT() /TeV, weight);
      }
      
      _h_MT->fill( sqrt( finalstate.Et2() - finalstate.pT2() ) /TeV, weight); 	
      _h_MT_dilep->fill(mT_dilep, weight);
     
      if ( !isZ ){
        _h_met->fill( neutrino.Et() /TeV, weight);
        _h_met_phi->fill( acos(cos(neutrino.phi())), weight);
      }

      _h_PtBal->fill( fabs(dijet.pT() - boson.pT()) /TeV, weight);
      _h_mll->fill( sqrt(fabs(boson.mass2())) /TeV, weight);
      _h_boson_pT->fill( boson.pT() /TeV, weight);
      _h_RATIOboson_pT->fill( boson.pT() /TeV, weight);
      _h_ALTboson_pT->fill( boson.pT() /TeV, weight);
	
      _h_deltaPhiJetLep->fill( dijet.phi() - boson.phi() , weight);
      
      double tot_jet_mass = 0;
      for( unsigned int i = 0 ; i<my_jets.size()-1 ; i++)
        tot_jet_mass += sqrt(fabs(my_jets[i].momentum().mass2())) /TeV;
      _h_totVisibleM->fill( tot_jet_mass, weight);
        
      _h_totVBFM->fill( sqrt(fabs((dijet+boson).mass2())) /TeV, weight);
	...
	for (uint j=i+1;j<my_jets.size();j++) {
	  ...
	  currentMass = sqrt(fabs((jeti + jetj).mass2())) /TeV;
...
os.system( "cd /data/atlas/atlasdata/henderson/custom_rivet; ./compile.sh")
